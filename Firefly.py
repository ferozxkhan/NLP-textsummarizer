...
class FireflyOptimizer(object):
    ...
    def _inilialize_population(self):
        ...
        print(self._fireflies)
        # for firefly in self._fireflies:
        #     print self._create_summary(firefly)
        # print "Done"
    ...
    def run_firefly(self, iteration):
        ...
        for _ in range(iteration):
            ...
            firefly_best = firefly_best + 1 * ( np.random.rand(len(self._sentences)) - 0.5 )
    ...
    def _move_firefly(self, i, j):
        ...
        new_firefly = firefly_i + 1 * math.e**(-1 * self._distance(firefly_i, firefly_j)**2) * (firefly_j - firefly_i) + 1 * ( np.random.rand(len(self._sentences)) - 0.5 )
        print (new_firefly)
        normalized_new_firefly = self._normalize(new_firefly)
        print (normalized_new_firefly) 
        sys_summary = []
        index = 0
        # print new_firefly
        ...
        self._fireflies[i] = normalized_new_firefly
        # print "Self"
        # print self._fireflies[i]
        # quit()
